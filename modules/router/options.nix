...
let
    ...
    # WAN CAKE configuration
    wanCakeConfigType = submodule {
    ...
    # WAN static addressing configuration
    wanStaticConfigType = submodule {
        ipv4 = {
            address = mkOption {
                type = types.str;
                default = "203.0.113.2";
    ...
    wanConfigType = submodule {
        ...
        static = mkOption {
            description = "Configuration for static addressing on the WAN interface, if `wan.type` == `static`";
            type = wanStaticConfigType;
        };
        pppoe = mkOption {
            description = "Configuration for PPPOE addressing on the WAN interface, if `wan.type` == `pppoe`";
            type = wanPPPOEConfigType;
        };
    };

    # Port or port range
    portSpecType = types.either types.port (submodule {
        from = mkOption {
            type = types.port;
    ...
    portForwardingConfigType = submodule {
        protocol = mkOption {
    ...
    # LAN isolation configuration type
    lanIsolationConfigType = submodule {
    ...
    # LAN DHCP configuration type
    lanNetworkDHCPConfigType = submodule {
    ...
    # LAN DNS configuration type
    lanNetworkDNSConfigType = submodule {
    ...
    # Configuration of individual LAN networks
    lanNetworkConfigType = types.attrsOf (
        types.submodule (
            { config, ... }:
            {
                options = {
                    ...
                    dhcp = mkOption {
                        ...
                        type = lanNetworkDHCPConfigType;
                    };
                    dns = mkOption {
                        description = "DNS configuration for this network";
                        type = lanNetworkDNSConfigType;
                    };
                };
            }
        )
    ...
    # LAN configuration type
    lanConfigType = submodule {
        isolation = mkOption {
            description = "Configuration of isolation between networks";
            type = lanIsolationConfigType;
            default = {
                enable = true;
            };
        };
    ...
    options = {
        nixos-utilities.systems.router = {
            ...
            config = {
                ...
                wan = mkOption {
                    description = "WAN interface & behavior config";
                    type = wanConfigType;
                };
                lan = mkOption {
                    description = "LAN configuration, including arbitrary subnets and subnet isolation";
                    type = lanConfigType;
                };
